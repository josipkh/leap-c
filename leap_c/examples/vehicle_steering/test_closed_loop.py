...
def visualize_closed_loop(results: np.ndarray, env: VehicleSteeringEnv) -> None:
    ...
    ax[1].plot(x, e2)
    ax[1].set_ylabel('$e_\psi$ [rad]')
    ax[2].plot(x, results[:, 1])
    ax[2].set_ylabel('$\dot{e}_y$ [m/s]')
    ax[3].plot(x, results[:, 3])
    ax[3].set_ylabel('$\dot{e}_\psi$ [rad/s]')
    ax[4].plot(x, delta)
    ...
    ax[1].plot(x, psi)
    ax[1].plot(x, psi_ref, 'r--')    
    ax[1].set_ylabel('$\psi$ [rad]')
