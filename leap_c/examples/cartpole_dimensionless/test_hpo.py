...
output_path = Path(f"/tmp/cartpole_dimensionless/sac_fop_{args.seed}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}")
...
def run_learning(lr_pi, lr_q, lr_alpha, init_alpha):
    ...
    main("sac_fop", "cartpole_swingup_dimensionless", cfg, output_path, args.device)
